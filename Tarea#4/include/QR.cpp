...
/**
 * Clase encargada de realizar el método de descomposición QR de una matriz
 */
...
public:
    ...
    void qr(const anpi::Matrix<T> &A, anpi::Matrix<T> &Q, anpi::Matrix<T> &R){
        ...
        Qn = A;
        anpi::Matrix<T> roots[size_x-1];
        for(int i=0;i<size_x-1;++i){
            if(i>0){Qn = getReducida(Qn);}
            temp = Qn;
            Qn = getTranspuesta(Qn);
            anpi::Matrix<T> iden;
            iden = getIdentidad(Qn.rows());
            Vector = getFila(Qn,0);
            ...
            v = iden - v;
            u = getCompleta(v, getIdentidad(size_x));
            roots[i] = u;
            Qn = v*temp;
        }
        anpi::Matrix<T> res;
