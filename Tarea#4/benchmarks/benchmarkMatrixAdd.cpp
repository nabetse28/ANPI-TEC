...
/// Benchmark for addition operations
    ...
    template<typename T>
    class benchLUDoolittle : public benchAdd<T> {
    public:
        /// Constructor
        benchLUDoolittle(const size_t n) : benchAdd<T>(n) { }
...
/// Provide the evaluation method for Crout
    template<typename T>
    class benchLUCrout : public benchAdd<T> {
    public:
        /// Constructor
        benchLUCrout(const size_t n) : benchAdd<T>(n) { }
